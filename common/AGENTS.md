...
**IMPORTANT**: All development must be done in **English**. Source code, field labels, help texts, UI strings, and comments must be written in English. German translations are then provided through `i18n/de.po` translation files.
...
# Export translations for a module (creates/updates de.po file)
...
1. Develop all features in English (code, labels, help texts)
2. Export translation template: `odoo-dev-bin -d dev_ee --language=de_DE --i18n-export=/workspace/labidoo/module_name/i18n/de.po --modules=module_name`
3. Translate strings in the `i18n/de.po` file
4. Import or restart Odoo to apply translations
...
7. **Development Language**: **ALWAYS develop in English**. All code, comments, labels, help texts, and user-facing strings must be in English. German translations are then provided separately in `i18n/de.po` files within each module.
...
**IMPORTANT**: NEVER mention AI authorship in commit messages. No "Co-Authored-By", "Generated by AI", or similar references.

### Commit Message Format

Follow the Odoo commit message convention:
...
| `[UPD]` | Updates (dependencies, configurations) |
| `[REF]` | Code refactoring (no functional change) |
...
**Simple fix:**
```
[FIX] labidoo_sale: correct discount calculation for subscription products
```

**New feature with description:**
```
[ADD] labidoo_mail: comprehensive test coverage for email processing

Add 62 new tests across 7 test files to improve test coverage:

- test_email_sanitization: 13 tests for ResPartner email sanitization
- test_mail_message_computed: 14 tests for body_stripped field
- test_eml_generation: 8 tests for EML file generation
```

**Multiple modules:**
```
[FIX] labidoo_sale, labidoo_account: align invoice date handling
```
...
# Check recent commit style for reference
git log --oneline -10
```
